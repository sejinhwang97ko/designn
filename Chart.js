...
function Chart() {
  ...
  const [ss,setSs] = useState("");
    ...
    <div className={"row"} style ={{ marginLeft:"2%"}}>
            ...
            <div style = {{ height: "800px", overflow: "scroll"}}><HospitalTable hospital={hos} ss={ss}/></div>
        </div>)}
        //음악리스트 큰 틀
function HospitalTable(props) {
    const row=[];
    props.hospital.forEach((m)=>{
        if(m.진료과목내용명.indexOf(props.ss)==-1 && (m.사업장명.indexOf(props.ss)==-1)&& (m.도로명전체주소.indexOf(props.ss)==-1)){
            return;
        }
...
function SearchBar(props) {
    ...
    return(
        <table className={"table"}>
            <tr>
                <td>
                    <input type={"text"} size={"25"} className={"input-sm"} placeholder={"병원에 대한 정보를 입력하세요!"} onChange={onChange}
                           value={props.ss}/>
                </td>
            </tr>
        </table>
